...
- **Dashboard "My tasks"**: pending-approval tasks only surface while the change is actually in `awaiting_approval` (approval rows exist from draft time and sibling rows stay pending after a rejection — neither counts as a task).
